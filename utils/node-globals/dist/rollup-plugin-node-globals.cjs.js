...
function inject (code, id, mod1, mod2, sourceMap) {
  ...
  function handleReference(node, name, keypath, parent) {
    if ((mod1.has(keypath) || mod2.has(keypath)) && !scope.contains(name) && !imports[name]) {
      ...
      /*
      if (moduleName.search('global.js') !== -1) {
        console.log("self reference", moduleName);
        return;
      }
      */
      ...
      if (!newImports[hash]) {
        newImports[hash] = 'import ' + (typeof module === 'string' ? importLocalName : '{ ' + module[1] + ' as ' + importLocalName + ' }') + ' from \'' + moduleName.replace(/(\\)/g, '/') + '\'';
        console.log(newImports[hash]);
      }
...
var GLOBAL_PATH = path.join(__dirname, 'utils', 'node-globals', 'src', 'global.js');
  ...
  return {
    ...
    resolveId: function resolveId(importee, importer) {
      if (importee === '__dirname') {
        ...
        console.log(path.relative(basedir, importer));
        return id;
      }
